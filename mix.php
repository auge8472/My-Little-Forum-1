...
$subnav_2 = '';
if (isset($_SESSION[$settings['session_prefix'].'user_id'])) $subnav_2 .= '<a href="index.php?update=1&amp;view=mix&amp;category='. intval($category) .'"><img src="'. $settings['themepath'] .'/img/update.gif" alt="'. htmlsc($lang['update_time_linktitle']) .'" width="9" height="9" /></a>';
...
if($thread_count > 0 && isset($result))
 {
  ...
  if ($zeile["fixed"] == 1) { ?> <img src="<?php echo $settings['themepath']; ?>/img/fixed.gif" width="9" height="9" title="<?php echo $lang['fixed']; ?>" alt="*" /><?php }
  if ($settings['all_views_direct'] == 1) { echo ' <span class="small">'; if ($settings['board_view'] == 1) { ?><a href="board_entry.php?id=<?php echo intval($zeile["tid"]); ?>"><img src="<?php echo $settings['themepath']; ?>/img/board_d.gif" alt="[Board]" title="<?php echo htmlsc($lang['open_in_board_linktitle']); ?>" width="12" height="9" /></a><?php } if ($settings['thread_view']==1) {?><a href="forum_entry.php?id=<?php echo intval($zeile["tid"]); ?>"><img src="<?php echo $settings['themepath']; ?>/img/thread_d.gif" alt="[Thread]" title="<?php echo htmlsc($lang['open_in_thread_linktitle']); ?>" width="12" height="9" /></a><?php } echo "</span>"; }
      ...
      <?php } ?>
      <td class="<?php if($i % 2 == 0) echo "a"; else echo "b"; ?>"><?php if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $zeile["user_id"] > 0) { ?><a href="user.php?id=<?php echo intval($zeile["user_id"]); ?>"><?php } ?><span class="small"><?php echo htmlsc($zeile["name"]); ?></span><?php if ($zeile["user_id"] > 0 && $settings['show_registered'] ==1) { ?><img src="<?php echo $settings['themepath']; ?>/img/registered.gif" alt="<?php echo htmlsc($lang['registered_user_title']); ?>" width="10" height="10" /><?php } if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $zeile["user_id"] > 0) { ?></a><?php } ?></td>
     ...
     if(isset($_SESSION[$settings['session_prefix'].'user_type']) && $_SESSION[$settings['session_prefix'].'user_type']=='admin')
      {
       ?><p class="marked-threads-board"><img src="'. $settings['themepath'] .'/img/marked.gif" alt="[x]" width="9" height="9" /> <?php echo $lang['marked_threads_actions']; ?> <a href="admin.php?action=delete_marked_threads&amp;refer=mix"><?php echo $lang['delete_marked_threads']; ?></a> - <a href="admin.php?action=lock_marked_threads&amp;refer=mix"><?php echo $lang['lock_marked_threads']; ?></a> - <a href="admin.php?action=unlock_marked_threads&amp;refer=mix"><?php echo $lang['unlock_marked_threads']; ?></a> - <a href="admin.php?action=unmark&amp;refer=mix"><?php echo $lang['unmark_threads']; ?></a> - <a href="admin.php?action=invert_markings&amp;refer=mix"><?php echo $lang['invert_markings']; ?></a> - <a href="admin.php?action=mark_threads&amp;refer=mix"><?php echo $lang['mark_threads']; ?></a></p><?php
      }
