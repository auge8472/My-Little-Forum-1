...
if ($settings['access_for_users_only'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_name']) || $settings['access_for_users_only'] != 1) {
	...
	if (isset($id) && $id > 0) {
		$result = mysqli_query($connid, "SELECT id, pid, tid, user_id, UNIX_TIMESTAMP(time + INTERVAL ". $time_difference ." HOUR) AS p_time, UNIX_TIMESTAMP(time) AS time, UNIX_TIMESTAMP(edited + INTERVAL ". $time_difference ." HOUR) AS e_time, UNIX_TIMESTAMP(edited - INTERVAL ". $settings['edit_delay'] ." MINUTE) AS edited_diff, edited_by, user_id, name, email, subject, hp, place, ip, text, show_signature, category, locked, ip FROM ". $db_settings['forum_table'] ." WHERE id = ". intval($id));
	...
	$result = mysqli_query($connid, "SELECT id, pid, tid, user_id, UNIX_TIMESTAMP(time) AS time, UNIX_TIMESTAMP(time + INTERVAL ". $time_difference ." HOUR) AS tp_time, UNIX_TIMESTAMP(last_answer) AS last_answer, name, subject, category, marked FROM ". $db_settings['forum_table'] ." WHERE tid = ". intval($Thread) ." ORDER BY time ASC");
	if(!$result) die($lang['db_error']);
...
$subnav_2 = "";
if ($settings['board_view']==1) $subnav_2 .= '<span class="small"><a href="board_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;category='. intval($category) .'"><img src="'. $settings['themepath'] .'/img/board.gif" alt="" width="12" height="9" />'.$lang['board_view_linkname'].'</a></span>';
if ($settings['mix_view']==1) $subnav_2 .= '&nbsp;&nbsp;<span class="small"><a href="mix_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;category='. intval($category) .'"><img src="'. $settings['themepath'] .'/img/mix.gif" alt="" />'.$lang['mix_view_linkname'].'</a></span>';
parse_template();
echo $header;
...
if ($entrydata["hp"]!="") { if (mb_substr($entrydata["hp"], 0, 7) != "http://" && mb_substr($entrydata["hp"], 0, 8) != "https://" && mb_substr($entrydata["hp"], 0, 6) != "ftp://" && mb_substr($entrydata["hp"], 0, 9) != "gopher://" && mb_substr($entrydata["hp"], 0, 7) != "news://") $entrydata["hp"] = "http://".$entrydata["hp"]; $email_hp .= '<a href="' . $entrydata["hp"] .'"><img src="'. $settings['themepath'] .'/img/homepage.gif" alt="'. htmlsc($lang['homepage_alt']) .'" width="13" height="13" /></a>'; }
if (($entrydata["email"]!="" && $entrydata["hide_email"] != 1) && $entrydata["hp"]!="") { $email_hp .= "&nbsp;"; }

if ($entrydata["email"]!="" && $entrydata["hide_email"] != 1 && isset($page) && isset($order) && isset($category)) { $email_hp .= '<a href="contact.php?id='. intval($entrydata["id"]) .'&amp;page='. intval($page) .'&amp;category='. intval($category) .'&amp;order='. urlencode($order) .'"><img src="'. $settings['themepath'] .'/img/email.gif" alt="'. htmlsc($lang['email_alt']) .'" title="'. str_replace("[name]", htmlsc($entrydata["name"]), $lang['email_to_user_linktitle']) .'" width="13" height="10" /></a>'; }
elseif ($entrydata["email"]!="" && $entrydata["hide_email"] != 1) { $email_hp .= '<a href="contact.php?id='. intval($entrydata["id"]) .'" title="'. str_replace("[name]", htmlsc($entrydata["name"]), $lang['email_to_user_linktitle']) .'"><img src="'. $settings['themepath'] .'/img/email.gif" alt="'. htmlsc($lang['email_alt']) .'" width="16" height="16" /></a>'; }
...
if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
 {
  ...
  $lang['forum_author_marking'] = str_replace("[name]", '<a href="user.php?id='. intval($entrydata["user_id"]) .'"><b>'. $name .'</b><img src="'. $settings['themepath'] .'/img/registered.gif" alt="'. htmlsc($lang['registered_user_title']) .'" width="10" height="10" /></a>', $lang['forum_author_marking']);
 }
...
elseif (!isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
 {
  $lang['forum_author_marking'] = str_replace("[name]", $name .'<img src="'. $settings['themepath'] .'/img/registered.gif" alt="'. htmlsc($lang['registered_user_title']) .'" width="10" height="10" />', $lang['forum_author_marking']);
 }
else
 {
