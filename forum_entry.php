...
$subnav_1 = '<a class="textlink" href="forum.php?page='. intval($page) .'&amp;category='. intval($category) .'&amp;order='. urlencode($order) .'" title="'. htmlsc($lang['back_to_forum_linktitle']).'">'.$lang['back_to_forum_linkname'].'</a>';
$subnav_2 = "";
if ($settings['board_view']==1) $subnav_2 .= '<span class="small"><a href="board_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;category='. intval($category) .'" title="'. htmlsc($lang['board_view_linktitle']) .'"><img src="img/board.gif" alt="" width="12" height="9" />'.$lang['board_view_linkname'].'</a></span>';
if ($settings['mix_view']==1) $subnav_2 .= '&nbsp;&nbsp;<span class="small"><a href="mix_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;category='. intval($category) .'" title="'. htmlsc($lang['mix_view_linktitle']) .'"><img src="img/mix.gif" alt="" />'.$lang['mix_view_linkname'].'</a></span>';
parse_template();
echo $header;
...
if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
 {
  ...
  $lang['forum_author_marking'] = str_replace("[name]", '<a href="user.php?id='. intval($entrydata["user_id"]) .'" title="'. htmlsc($lang['show_userdata_linktitle']) .'"><b>'. $name .'</b><img src="img/registered.gif" alt="(R)" width="10" height="10" title="'. htmlsc($lang['registered_user_title']) .'" /></a>', $lang['forum_author_marking']);
 }
elseif (isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] !=1)
 {
  $lang['show_userdata_linktitle'] = str_replace("[name]", htmlsc($entrydata["name"]), $lang['show_userdata_linktitle']);
  $lang['forum_author_marking'] = str_replace("[name]", '<a href="user.php?id='. intval($entrydata["user_id"]) .'" title="'. htmlsc($lang['show_userdata_linktitle']) .'"><b>'. $name .'</b></a>', $lang['forum_author_marking']);
 }
elseif (!isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
 {
  $lang['forum_author_marking'] = str_replace("[name]", $name .'<img src="img/registered.gif" alt="(R)" width="10" height="10" title="'. htmlsc($lang['registered_user_title']) .'" />', $lang['forum_author_marking']);
 }
else
 {
