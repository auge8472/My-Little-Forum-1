...
$subnav_2 = "";
if ($settings['board_view']==1) $subnav_2 .= '<span class="small"><a href="board_entry.php?id='.$entrydata["tid"].'&amp;page='.$page.'&amp;order='.$order.'&amp;category='.urlencode($category).'" title="'.$lang['board_view_linktitle'].'"><img src="img/board.gif" alt="" width="12" height="9" title="'.$lang['board_view_linktitle'].'" />'.$lang['board_view_linkname'].'</a></span>';
if ($settings['mix_view']==1) $subnav_2 .= '&nbsp;&nbsp;<span class="small"><a href="mix_entry.php?id='.$entrydata["tid"].'&amp;page='.$page.'&amp;order='.$order.'&amp;category='.urlencode($category).'" title="'.$lang['mix_view_linktitle'].'"><img src="img/mix.gif" alt="" width="12" height="9" title="'.$lang['mix_view_linktitle'].'" />'.$lang['mix_view_linkname'].'</a></span>';
parse_template();
echo $header;
?><h2 class="postingheadline"><?php echo htmlspecialchars(stripslashes($entrydata["subject"])); ?><?php if (isset($categories[$entrydata["category"]]) && $categories[$entrydata["category"]]!='') { ?> <span class="category">(<?php echo $categories[$entrydata["category"]]; ?>)</span><?php } ?></h2>
<?php
...
if ($entrydata["hp"]!="") { if (substr($entrydata["hp"],0,7) != "http://" && substr($entrydata["hp"],0,8) != "https://" && substr($entrydata["hp"],0,6) != "ftp://" && substr($entrydata["hp"],0,9) != "gopher://" && substr($entrydata["hp"],0,7) != "news://") $entrydata["hp"] = "http://".$entrydata["hp"]; $email_hp .= "<a href=\"" . $entrydata["hp"] ."\" title=\"".htmlspecialchars(stripslashes($entrydata["hp"]))."\"><img src=\"img/homepage.gif\" alt=\"".$lang['homepage_alt']."\" width=\"13\" height=\"13\" /></a>"; }
if (($entrydata["email"]!="" && $entrydata["hide_email"] != 1) && $entrydata["hp"]!="") { $email_hp .= "&nbsp;"; }

if ($entrydata["email"]!="" && $entrydata["hide_email"] != 1 && isset($page) && isset($order) && isset($category)) { $email_hp .= '<a href="contact.php?id='.$entrydata["id"].'&amp;page='.$page.'&amp;category='.urlencode($category).'&amp;order='.$order.'"><img src="img/email.gif" alt="'.$lang['email_alt'].'" title="'.str_replace("[name]", htmlspecialchars(stripslashes($entrydata["name"])), $lang['email_to_user_linktitle']).'" width="13" height="10" /></a>'; }
elseif ($entrydata["email"]!="" && $entrydata["hide_email"] != 1) { $email_hp .= '<a href="contact.php?id='.$entrydata["id"].'" title="'.str_replace("[name]", htmlspecialchars(stripslashes($entrydata["name"])), $lang['email_to_user_linktitle']).'"><img src="img/email.gif" alt="'.$lang['email_alt'].'" width="16" height="16" /></a>'; }

if ($entrydata["place"] != "") { $place_c = htmlspecialchars(stripslashes($entrydata["place"])) . ", "; $place = htmlspecialchars(stripslashes($entrydata["place"])); }

if ($mark_admin==true) $name = "<span class=\"admin-highlight\">".htmlspecialchars(stripslashes($entrydata["name"]))."</span>";
elseif ($mark_mod==true) $name = "<span class=\"mod-highlight\">".htmlspecialchars(stripslashes($entrydata["name"]))."</span>";
...
if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
 {
  $lang['show_userdata_linktitle'] = str_replace("[name]", htmlspecialchars(stripslashes($entrydata["name"])), $lang['show_userdata_linktitle']);
...
elseif (!isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0 && $settings['show_registered'] ==1)
...
else
 {
  $lang['forum_author_marking'] = str_replace("[name]", $name, $lang['forum_author_marking']);
   ...
   $lang['forum_edited_marking'] = str_replace("[name]", htmlspecialchars(stripslashes($entrydata["edited_by"])), $lang['forum_edited_marking']);
