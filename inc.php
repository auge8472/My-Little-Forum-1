...
if (empty($_SESSION[$settings['session_prefix']."user_id"]) && $settings['remember_last_visit'] == 1)
	...
	if (isset($_COOKIE['last_visit']))
		...
		if (isset($c_last_visit[0])) $c_last_visit[0] = trim($c_last_visit[0]);
		else $c_last_visit[0] = time();
		if (isset($c_last_visit[1])) $c_last_visit[1] = trim($c_last_visit[1]);
		else $c_last_visit[1] = time();
...
if (isset($c_last_visit)) $last_visit = $c_last_visit[0];
...
else if (is_array($categories)) // there are categories
	{
