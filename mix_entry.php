      ...
      if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] > 0)
       {
        ...
        ?><a id="<?php echo "p". intval($entrydata["id"]); ?>" href="user.php?id=<?php echo intval($entrydata["user_id"]); ?>" title="<?php echo htmlsc($show_userdata_linktitle_x); ?>"><b><?php echo htmlsc($entrydata["name"]); ?></b><?php if ($settings['show_registered'] ==1) { ?><img src="img/registered.gif" alt="(R)" width="10" height="10" title="<?php echo htmlsc($lang['registered_user_title']); ?>" /><?php } ?></a><br /><?php
       }
      // ansonsten nur den Namen anzeigen:
      else
       {
        ?>
        <a id="<?php echo "p". intval($entrydata["id"]); ?>"><b><?php echo htmlsc($entrydata["name"]); ?></b><?php if ($entrydata["user_id"] > 0 && $settings['show_registered'] ==1) { ?><img src="img/registered.gif" alt="(R)" width="10" height="10" title="<?php echo htmlsc($lang['registered_user_title']); ?>" /><?php } ?></a><br />
        <?php
       }
        ...
        if ($entrydata["email"]!="" && $entrydata["hide_email"] != 1 && isset($page) && isset($order) && isset($category)) { echo '<a href="contact.php?id='. intval($entrydata["id"]) .'&amp;page='. intval($page) .'&amp;category='. intval($category) .'&amp;order='. urlencode($order) .'&amp;view=mix" title="'. str_replace("[name]", htmlsc($entrydata["name"]), $lang['email_to_user_linktitle']) .'"><img src="img/email.gif" alt="'. htmlsc($lang['email_alt']) .'" width="13" height="10" /></a>'; }
        elseif ($entrydata["email"]!="" && $entrydata["hide_email"] != 1) { echo '<a href="contact.php?id='. intval($entrydata["id"]) .'&amp;view=mix" title="'. str_replace("[name]", htmlsc($entrydata["name"]), $lang['email_to_user_linktitle']) .'"><img src="img/email.gif" alt="'. htmlsc($lang['email_alt']) .'" width="13" height="10" /></a>'; }
        if (($entrydata["email"]!="" && $entrydata["hide_email"] != 1) or $entrydata["hp"]!="") { echo "<br />"; }
        echo "<br />";
        ...
        <?php
        if ($settings['user_edit'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] == $_SESSION[$settings['session_prefix']."user_id"] || isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "admin" || isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "mod") { ?><br /><br /><span class="small"><a href="posting.php?action=edit&amp;id=<?php echo intval($entrydata["id"]); ?>&amp;view=mix&amp;back=<?php echo intval($entrydata["tid"]); ?>&amp;page=<?php echo intval($page); ?>&amp;order=<?php echo urlencode($order); ?>&amp;descasc=<?php echo urlencode($descasc); ?>&amp;category=<?php echo intval($category); ?>" title="<?php echo htmlsc($lang['edit_linktitle']); ?>"><img src="img/edit.gif" alt="" width="15" height="10" /><?php echo $lang['edit_linkname']; ?></a></span><?php } if ($settings['user_delete'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_id']) && $entrydata["user_id"] == $_SESSION[$settings['session_prefix']."user_id"] || isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "admin" || isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "mod") { ?><br /><span class="small"><a href="posting.php?action=delete&amp;id=<?php echo intval($entrydata["id"]); ?>&amp;back=<?php echo intval($entrydata["tid"]); ?>&amp;view=mix&amp;page=<?php echo intval($page); ?>&amp;order=<?php echo urlencode($order); ?>&amp;descasc=<?php echo urlencode($descasc); ?>&amp;category=<?php echo urlencode($category); ?>" title="<?php echo htmlsc($lang['delete_linktitle']); ?>"><img src="img/delete.gif" alt="" width="12" height="9" /><?php echo $lang['delete_linkname']; ?></a></span><?php }
        if (isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "admin" && $entrydata['pid'] == 0 || isset($_SESSION[$settings['session_prefix'].'user_id']) && $_SESSION[$settings['session_prefix']."user_type"] == "mod" && $entrydata['pid'] == 0) { ?><br /><span class="small"><a href="posting.php?lock=true&amp;view=mix&amp;id=<?php echo intval($entrydata["id"]); ?>&amp;page=<?php echo intval($page); ?>&amp;order=<?php echo urlencode($order); ?>&amp;descasc=<?php echo urlencode($descasc); ?>&amp;category=<?php echo intval($category); ?>" title="<?php if ($entrydata['locked'] == 0) echo htmlsc($lang['lock_linktitle']); else echo htmlsc($lang['unlock_linktitle']); ?>"><img src="img/lock.gif" alt="" width="12" height="12" /><?php if ($entrydata['locked'] == 0) echo $lang['lock_linkname']; else echo $lang['unlock_linkname']; ?></a></span><?php }
        ?><div class="autorcellwidth">&nbsp;</div></td>
     <td class="titlecell" valign="top"><div class="left"><h2><?php echo htmlsc($entrydata["subject"]); if(isset($categories[$entrydata["category"]]) && $categories[$entrydata["category"]]!='' && $entrydata["pid"]==0) echo '&nbsp;<span class="category">('. $categories[$entrydata["category"]] .')</span>'; ?></h2></div><div class="right"><?php if ($entrydata['locked'] == 0) { ?><a class="textlink" href="posting.php?id=<?php echo intval($entrydata["id"]); if (isset($page) && isset($order) && isset($descasc) && isset($category)) { ?>&amp;page=<?php echo intval($page); ?>&amp;category=<?php echo intval($category); ?>&amp;order=<?php echo urlencode($order); ?>&amp;descasc=<?php echo urlencode($descasc); } ?>&amp;view=mix" title="<?php echo htmlsc($lang['board_answer_linktitle']); ?>"><?php echo $lang['board_answer_linkname']; ?></a><?php } else { if ($entrydata['pid']==0) { ?><span class="xsmall"><img src="img/lock.gif" alt="" width="12" height="12" /><?php echo $lang['thread_locked']; ?></span><?php } else echo "&nbsp;"; } ?></div></td>
    </tr>
    <tr>
...
$subnav_2 = "";
if ($settings['thread_view']==1) $subnav_2 .= '<span class="small"><a href="forum_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;descasc='. urlencode($descasc) .'&amp;category='. intval($category) .'" title="'. htmlsc($lang['thread_view_linktitle']) .'"><img src="img/thread.gif" alt="" />'.$lang['thread_view_linkname'].'</a></span>';
if ($settings['board_view']==1) $subnav_2 .= '&nbsp;&nbsp;<span class="small"><a href="board_entry.php?id='. intval($entrydata["tid"]) .'&amp;page='. intval($page) .'&amp;order='. urlencode($order) .'&amp;category='. urlencode($category) .'" title="'. htmlsc($lang['board_view_linktitle']) .'"><img src="img/board.gif" alt="" width="12" height="9" />'.$lang['board_view_linkname'].'</a></span>';
