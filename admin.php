...
function sql_forum() {
	...
	while ($field = mysqli_fetch_assoc($sql_result)) {
		echo "INSERT INTO ".$db_settings['forum_table']." VALUES (". intval($field['id']) .", ". intval($field['pid']) .", ". intval($field['tid']) .", '". mysqli_real_escape_string($connid, $field['uniqid']) ."', ". intval($field['time']) .", '". mysqli_real_escape_string($connid, $field['last_answer']) ."', '". mysqli_real_escape_string($connid, $field['edited']) ."', '". mysqli_real_escape_string($connid, $field['edited_by']) ."', ". intval($field['user_id']) .", '". mysqli_real_escape_string($connid, $field['name']) ."', '". mysqli_real_escape_string($connid, $field['subject']) ."', ". intval($field['category']) .", '". mysqli_real_escape_string($connid, $field['email']) ."', '". mysqli_real_escape_string($connid, $field['hp']) ."', '". mysqli_real_escape_string($connid, $field['place']) ."', '". mysqli_real_escape_string($connid, $field['ip']) ."', '". mysqli_real_escape_string($connid, $field['text']) ."', ". intval($field['show_signature']) .", ". intval($field['email_notify']) .", ". intval($field['marked']) .", ". intval($field['locked']) .", ". intval($field['fixed']) .", ". intval($field['views']) .");<br />";
	}
...
function sql_forum_marked() {
	...
	while ($field = mysqli_fetch_assoc($sql_result)) {
		echo "INSERT INTO ".$db_settings['forum_table']." VALUES (". intval($field['id']) .", ". intval($field['pid']) .", ". intval($field['tid']) .", '". mysqli_real_escape_string($connid, $field['uniqid']) ."', '". mysqli_real_escape_string($connid, $field['time']) ."', '". mysqli_real_escape_string($connid, $field['last_answer']) ."', '". mysqli_real_escape_string($connid, $field['edited']) ."', '". mysqli_real_escape_string($connid, $field['edited_by']) ."', ". intval($field['user_id']) .", '". mysqli_real_escape_string($connid, $field['name']) ."', '". mysqli_real_escape_string($connid, $field['subject']) ."', ". intval($field['category']) .", '". mysqli_real_escape_string($connid, $field['email']) ."', '". mysqli_real_escape_string($connid, $field['hp']) ."', '". mysqli_real_escape_string($connid, $field['place']) ."', '". mysqli_real_escape_string($connid, $field['ip']) ."', '". mysqli_real_escape_string($connid, $field['text']) ."', ". intval($field['show_signature']) .", ". intval($field['email_notify']) .", ". intval($field['marked']) .", ". intval($field['locked']) .", ". intval($field['fixed']) .", ". intval($field['views']) .");<br />";
	}
	mysqli_free_result($sql_result);
	?><br /></pre><?php
}
...
if (isset($_POST['new_category'])) {
	...
	if ($new_category!='') {
		...
		if (empty($errors)) {
			...
			mysqli_query($connid, "INSERT INTO ". $db_settings['category_table'] ." (category_order, category, accession) VALUES (". intval($category_count) ."+1,'". mysqli_real_escape_string($connid, $new_category) ."',". intval($accession).")");
...
if (isset($_POST['clear_userdata'])) {
	...
	if (isset($selected)) $action = "delete_users_sure";
	else {
...
if (isset($_POST['delete_db_confirmed'])) {
	...
	if (empty($errors)) {
		...
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['forum_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
		...
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['userdata_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		echo 'Deleting table <b>'.$db_settings['useronline_table'].'</b>... ';
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['useronline_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		echo 'Deleting table <b>'.$db_settings['settings_table'].'</b>... ';
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['settings_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		echo 'Deleting table <b>'.$db_settings['category_table'].'</b>... ';
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['category_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		echo 'Deleting table <b>'.$db_settings['smilies_table'].'</b>... ';
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['smilies_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		echo 'Deleting table <b>'.$db_settings['banlists_table'].'</b>... ';
		if (mysqli_query($connid, "DROP TABLE ". $db_settings['banlists_table'])) echo '<b style="color:green;">OK</b><br />';
		else {
			$errors[] = mysqli_error($connid);
			echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
		}
		...
		if ($_POST['delete_modus'] == "db") {
			...
			if (mysqli_num_rows($result) == 0) {
				if (mysqli_query($connid, "DROP DATABASE ". $db_settings['db'])) echo '<b style="color:green;">OK</b><br />';
				else {
					$errors[] = mysqli_error($connid);
					echo '<b style="color:red;">FAILED</b> (MySQL: '.mysqli_error($connid).')<br />';
				}
...
if (isset($_POST['delete_marked_threads_confirmed'])) {
	...
	else if (isset($_POST['refer']) && $_POST['refer'] == 'mix') header("location: mix.php");
...
if (isset($_POST['ar_username'])) {
	...
	if(empty($errors)) {
		...
		if (mb_strlen($ar_username) > $settings['name_maxlength']) $errors[] = $lang['name_marking'] . " " .$lang['error_input_too_long'];
		...
		if (!preg_match("/^[^@]+@.+\.\D{2,}$/", $ar_email)) $errors[] = $lang['error_email_wrong'];

		if ($ar_pw_conf != $ar_pw) $errors[] = $lang_add['error_pw_conf_wrong'];
	}
	// save the new user if no error occured:
	if (empty($errors)) {
		...
		if ($ar_pw == '') {
			...
			$ar_pw = "";
			for ($i = 0; $i < 8; $i++) {
				$ar_pw .= mb_substr($letters, mt_rand(0, mb_strlen($letters) - 1), 1);
			}
		}
...
switch ($action)
 {
  ...
  case "delete_users_sure":
   ...
   {
    ?><li><a href="user.php?id=<?php echo intval($selected[$x]); ?>"><b><?php echo htmlsc($selected_usernames[$x]); ?></b></a><?php
   }
   ?>
   </ul>
   <br />
