...
function category_accession() {
	...
	mysqli_free_result($result);
	if (isset($category_accession)) return $category_accession;
	return false;
}
...
function bbcode($string, $rss = false) {
	...
	$string = preg_replace("#\[inlinecode\](.+?)\[/inlinecode\]#is", "<code>\\1</code>", $string);
...
function parse_code($string) {
	if(basename($_SERVER['SCRIPT_NAME']) == 'board_entry.php' || basename($_SERVER['SCRIPT_NAME']) == 'mix_entry.php') $p_class='postingboard';
