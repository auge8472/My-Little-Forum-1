...
function zitat($string)
 ...
 function thread_tree($id, $aktuellerEintrag = 0, $tiefe = 0)
 {
  ...
  elseif ($mark_mod==true) $name = "<span class=\"mod-highlight\">".htmlspecialchars(stripslashes($parent_array[$id]["name"]))."</span>";
  ...
  elseif ($id == $aktuellerEintrag && $parent_array[$id]["pid"]!=0)
  ...
  else
   {
    ...
    else echo "reply"; ?>" href="forum_entry.php?id=<?php echo $parent_array[$id]["id"]; if ($page != 0 || $category != 0 || $order != "time") echo '&amp;page='.$page.'&amp;category='.urlencode(stripslashes($category)).'&amp;order='.$order; ?>"><?php echo htmlspecialchars(stripslashes($parent_array[$id]["subject"])); ?></a><?php
   }
  echo " " . $thread_info_b;
