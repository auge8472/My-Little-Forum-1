...
if (isset($_SESSION[$settings['session_prefix'].'user_id']))
 {
  $lock_result = mysqli_query($connid, "SELECT user_lock FROM ".$db_settings['userdata_table']." WHERE user_id = '".$_SESSION[$settings['session_prefix'].'user_id']."' LIMIT 1");
...
{
 $lock_result = mysqli_query($connid, "SELECT tid, locked FROM ".$db_settings['forum_table']." WHERE id='".$id."' LIMIT 1");
 if (!$lock_result) die($lang['db_error']);
 $field = mysqli_fetch_assoc($lock_result);
 mysqli_free_result($lock_result);
 if ($field['locked']==0) mysqli_query($connid, "UPDATE ".$db_settings['forum_table']." SET time=time, last_answer=last_answer, edited=edited, locked='1' WHERE tid = ".$field['tid']);
 else mysqli_query($connid, "UPDATE ".$db_settings['forum_table']." SET time=time, last_answer=last_answer, edited=edited, locked='0' WHERE tid = ".$field['tid']);
...
if ($settings['entries_by_users_only'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_name']) || $settings['entries_by_users_only'] != 1)
{
 ...
 if ($categories == "not accessible") { header("location: index.php"); die("<a href=\"index.php\">further...</a>"); }
    ...
    if ($action=="edit" || $action == "delete" || $action == "delete ok")
     {
      $user_id_result = mysqli_query($connid, "SELECT user_id FROM ".$db_settings['forum_table']." WHERE id = '".$id."' LIMIT 1");
      if (!$user_id_result) die($lang['db_error']);
      $result_array = mysqli_fetch_assoc($user_id_result);
      mysqli_free_result($user_id_result);

      $user_type_result = mysqli_query($connid, "SELECT user_type FROM ".$db_settings['userdata_table']." WHERE user_id = '".$result_array["user_id"]."' LIMIT 1");
   ...
   switch ($action)
    {
     case "new":
      ...
      if ($id != 0)
       {
        $result = mysqli_query($connid, "SELECT tid, pid, name, subject, category, text, locked FROM ".$db_settings['forum_table']." WHERE id = ".$id);
      ...
      if ($edit_authorization == 1)
       {
        ...
        $edit_result = mysqli_query($connid, "SELECT tid, pid, user_id, name, email, hp, place, subject, category, text, email_notify, show_signature, locked, fixed, UNIX_TIMESTAMP(time) AS time, UNIX_TIMESTAMP(NOW() - INTERVAL ".$settings['edit_period']." MINUTE) AS edit_diff FROM ".$db_settings['forum_table']." WHERE id = ".$id);
     ...
     case "delete":
      if ($delete_authorization == 1)
       {
        $delete_result = mysqli_query($connid, "SELECT tid, pid, UNIX_TIMESTAMP(time + INTERVAL ".$time_difference." HOUR) AS tp_time, name, subject, category FROM ".$db_settings['forum_table']." WHERE id = '".$id."'");
     ...
     case "delete ok":
       if ($delete_authorization == 1)
        {
         $pid_result=mysqli_query($connid, "SELECT pid FROM ".$db_settings['forum_table']." WHERE id = ".$id);
         if (!$pid_result) die($lang['db_error']);
         $feld = mysqli_fetch_assoc($pid_result);
         if ($feld["pid"] == 0)
          {
           $delete_result = mysqli_query($connid, "DELETE FROM ".$db_settings['forum_table']." WHERE tid = ".$id);
          }
         else
          {
           $last_answer_result=mysqli_query($connid, "SELECT tid, time, last_answer FROM ".$db_settings['forum_table']." WHERE id = ".$id);
           ...
           if ($field['time'] == $field['last_answer'])
            {
             ...
             $last_answer_result=mysqli_query($connid, "SELECT time FROM ".$db_settings['forum_table']." WHERE tid = ".$field['tid']." AND time < '".$field['time']."' ORDER BY time DESC LIMIT 1");
             $field2 = mysqli_fetch_assoc($last_answer_result);
             mysqli_free_result($last_answer_result);
             $update_result=mysqli_query($connid, "UPDATE ".$db_settings['forum_table']." SET time=time, last_answer='".$field2['time']."' WHERE tid=".$field['tid']);
            }
           // delete message:
           $delete_result = mysqli_query($connid, "DELETE FROM ".$db_settings['forum_table']." WHERE id = ".$id);
          }
 ...
 elseif (isset($form))
  {
   ...
   switch ($action)
    {
     case "new":
      ...
      if ($id != 0)
       {
        $tid_result = mysqli_query($connid, "SELECT tid, locked FROM ".$db_settings['forum_table']." WHERE id=".$id);
     ...
     case "edit";
      ...
      $edit_result = mysqli_query($connid, "SELECT name, locked, UNIX_TIMESTAMP(time) AS time, UNIX_TIMESTAMP(NOW() - INTERVAL ".$settings['edit_period']." MINUTE) AS edit_diff FROM ".$db_settings['forum_table']." WHERE id = ".$id);
      if (!$edit_result) die($lang['db_error']);
      $field = mysqli_fetch_assoc($edit_result);
      mysqli_free_result($edit_result);
   ...
   // check data:
        // double entry?
        $uniqid_result = mysqli_query($connid, "SELECT COUNT(*) FROM ".$db_settings['forum_table']." WHERE uniqid = '".$uniqid."' AND time > NOW()-10000");
     ...
     switch ($action)
      {
       case "new":
            ...
            if($id != 0) { if(!mysqli_query($connid, "UPDATE ".$db_settings['forum_table']." SET time=time, last_answer=NOW() WHERE tid=".$Thread))
            die($lang['db_error']); }
            ...
            // Schauen, ob eine E-Mail benachrichtigung versendet werden soll:
             if ($settings['email_notification'] == 1)
              {
               $parent_result = mysqli_query($connid, "SELECT user_id, name, email, subject, text, email_notify FROM ".$db_settings['forum_table']." WHERE id = '".$id."' LIMIT 1");
               ...
               if ($parent["email_notify"] == 1)
                {
                 ...
                 if ($parent["user_id"] > 0)
                  {
                   $email_result = mysqli_query($connid, "SELECT user_name, user_email FROM ".$db_settings['userdata_table']." WHERE user_id = '".$parent["user_id"]."' LIMIT 1");
        ...
        if ($edit_authorization == 1 && ($field['locked'] == 0 || (isset($_SESSION[$settings['session_prefix'].'user_type']) && ($_SESSION[$settings['session_prefix'].'user_type']=='admin' || $_SESSION[$settings['session_prefix'].'user_type']=='mod'))))
         {
          ...
          {
          $tid_result=mysqli_query($connid, "SELECT tid, name, subject, text FROM ".$db_settings['forum_table']." WHERE id = ".$id);
          if (!$tid_result) die($lang['db_error']);
          $field = mysqli_fetch_assoc($tid_result);
          mysqli_free_result($tid_result);
          ...
          $category_update_result = mysqli_query($connid, "UPDATE ".$db_settings['forum_table']." SET time=time, last_answer=last_answer, edited=edited, category=".intval($p_category)." WHERE tid = '".$field["tid"]."'");
     ...
     if(isset($_SESSION[$settings['session_prefix'].'user_id']))
      {
       ...
       $preview_result = mysqli_query($connid, "SELECT user_name, user_email, hide_email, user_hp, user_place, signature FROM ".$db_settings['userdata_table']." WHERE user_id = '".$pr_id."' LIMIT 1");
