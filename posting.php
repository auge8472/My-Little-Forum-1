...
if (($settings['access_for_users_only'] == 1
	...
	if (($settings['entries_by_users_only'] == 1
		...
		if (is_array($categories)
			and !in_array($_SESSION[$settings['session_prefix'].'category'], $categories))
			{
		...
		$topnav  = '<li><a href="'. $subnav['href'].$subnav['query'] .'"><span class="fa fa-chevron-right icon-chevron-left">';
		...
		switch ($show)
			...
			case "form":
				...
				if (isset($oldMessage['name']))
					...
					$tParentName = str_replace('{parentName}', !empty($oldMessage['name']) ? htmlspecialchars($oldMessage['name']) : '', $tParentName);
					}
