...
if ($settings['access_for_users_only'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_name']) || $settings['access_for_users_only'] != 1) {
	if ($settings['entries_by_users_only'] == 1 && isset($_SESSION[$settings['session_prefix'].'user_name']) || $settings['entries_by_users_only'] != 1) {
		...
		unset($Thread);
		if (empty($descasc)) $descasc="DESC";
		$edit_authorization = 0;    // initialise the edit authorisation for entries with no
		$delete_authorization = 0;  // initialise the delete authorisation for entries with no

		if (empty($action)) $action = "new";

		// check if the user is authorised to edit or delete a posting:
		if ($action == "edit" || $action == "delete" || $action == "delete ok") {
			...
			if (isset($_SESSION[$settings['session_prefix'].'user_id'])) {
				if ($_SESSION[$settings['session_prefix'].'user_type'] == "admin") {
					...
					// Moderator darf alles außer Postings von Admins editieren/lÃ¶schen:
		...
		if (empty($form)) {
			switch ($action) {
				case "new":
					...
					if ($id != 0) {
						...
						if ($field['locked'] > 0 && (empty($_SESSION[$settings['session_prefix'].'user_type']) || (isset($_SESSION[$settings['session_prefix'].'user_type']) && $_SESSION[$settings['session_prefix'].'user_type'] != 'admin' && $_SESSION[$settings['session_prefix'].'user_type'] != 'mod'))) {
				...
				case "edit":
					if ($edit_authorization == 1) {
						...
						else if ($settings['edit_period'] > 0 && $field["edit_diff"] > $field["time"] && (empty($_SESSION[$settings['session_prefix'].'user_type']) || (isset($_SESSION[$settings['session_prefix'].'user_type']) && $_SESSION[$settings['session_prefix'].'user_type'] != 'admin' && $_SESSION[$settings['session_prefix'].'user_type'] != 'mod'))) { $show = "no authorization"; $reason = str_replace('[minutes]',$settings['edit_period'],$lang['edit_period_over']); }
						else $show = "form";
					}
				...
				case "delete ok":
					if ($delete_authorization == 1) {
						...
						if (isset($page) && isset($order) && isset($category) && isset($descasc)) {
							$qs="?page=". intval($page)."&order=". urlencode($order) ."&descasc=". urlencode($descasc) ."&category=". intval($category);
						}
						else $qs = "";

						if (isset($view)) {
							if ($view == 'board') {
								header("location: board.php". $qs);
								die('<a href="board.php'. $qs .'">further...</a>');
							}
							else {
								header("location: mix.php".$qs);
								die('<a href="mix.php'. $qs .'">further...</a>');
							}
						} else {
							header("location: forum.php".$qs);
			...
			// form submitted:
			if (empty($_POST['fixed'])) $fixed = 0;
			else $fixed = $_POST['fixed'];
			switch ($action) {
				case "new":
			...
			if (empty($errors) && empty($preview) && isset($_POST['save_entry'])) {
				switch ($action) {
					case "new":
						...
						if($id != 0) {
							if (!mysqli_query($connid, "UPDATE ". $db_settings['forum_table'] ." SET time=time, last_answer=NOW() WHERE tid=". intval($Thread)))
								die($lang['db_error']);
						}
						...
						if ($settings['email_notification'] == 1) {
							...
							if ($parent["email_notify"] == 1) {
								...
								else if ($settings['standard'] == "mix") $emailbody = str_replace("[posting_address]", $settings['forum_address']."mix_entry.php?id=".$neu["tid"]."#p".$neu["id"], $emailbody);
						...
						#$ip = $_SERVER["REMOTE_ADDR"];
						$mail_text = unbbcode($text);
						if ($id > 0) $emailbody = str_replace("[name]", $name, $lang['admin_email_text_reply']);
			...
			if (isset($refer)) {
				...
				if (isset($view) && $view=="board") {
					header("location: board_entry.php?id=". intval($further_tid) . $qs);
					die('<a href="board_entry.php?id='. intval($further_tid) . $qs .'">further...</a>');
				} elseif (isset($view) && $view=="mix") {
					header("location: mix_entry.php?id=". intval($further_id) . $qs);
					die('<a href="mix_entry.php?id='. intval($further_tid) . $qs .'">further...</a>');
				} else {
					header("location: forum_entry.php?id=". intval($further_id) . $qs);
					die('<a href="forum_entry.php?id='. intval($further_id) . $qs .'">further...</a>');
				}
			exit(); // Skript beenden
			}
		} // End of "else if (isset(form))"

		switch ($action) {
			case "new":
				if ($id == 0) $wo = $lang['new_entry_marking'];
				else $wo = $lang['answer_marking'];
			break;
			case "edit";
...
switch ($show)
  ...
  break;
   ...
   break;
		...
		header("location: login.php?msg=noentry");
