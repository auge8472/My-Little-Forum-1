...
$user_time_difference = (isset($_COOKIE['user_time_difference'])) ? intval($_COOKIE['user_time_difference']) : 0;
...
if (isset($_SESSION[$settings['session_prefix'].'user_id'])) {
  ?><p class="posting"><?php echo htmlsc($lang_add['td_user_note']); ?></p><?php
} else {
  ?><p class="posting"><?php echo htmlsc($lang_add['td_desc']); ?></p>
  <form action="<?php echo basename($_SERVER["PHP_SELF"]); ?>" method="post"><div></div>
  <select name="user_time_difference"><?php
	for ($h = -24; $h <= 24; $h++) {
		$sel_val = ($user_time_difference==$h) ? ' selected="selected"' : '';
		?><option value="<?php echo $h; ?>"<?php $sel_val; ?>><?php echo $h; ?></option><?php
	} ?></select></td>
  <input type="submit" name="ok" value="<?php echo htmlsc($lang['submit_button_ok']); ?>" />
  </div></form><?php
}
echo $footer;
?>
