...
if (isset($_SESSION[$settings['session_prefix'].'user_id']) && empty($action)) {
	$action = "logout";
} else if (empty($_SESSION[$settings['session_prefix'].'user_id']) && isset($username) && $username != "" && isset($userpw) && $userpw != "") {
	$action = "login ok";
} else if (empty($_SESSION[$settings['session_prefix'].'user_id']) && isset($username) && isset($userpw) && ($username == ""  || $userpw == "")) {
	header("location: ". basename($_SERVER['SCRIPT_NAME']) ."?msg=login_failed");
	die('<a href="'. basename($_SERVER['SCRIPT_NAME']) .'?msg=login_failed">further...</a>');
} else if (empty($_SESSION[$settings['session_prefix'].'user_id']) && isset($_COOKIE['auto_login']) && isset($settings['autologin']) && $settings['autologin'] == 1) {
	$action = "auto_login";
} else if (empty($_SESSION[$settings['session_prefix'].'user_id']) && empty($action) && empty($_GET['activate'])) {
	$action = "login";
} else if (empty($_SESSION[$settings['session_prefix'].'user_id']) && empty($action) && isset($_GET['activate'])) {
	$action = "activate";
}
...
switch ($action) {
	...
	case "auto_login":
		...
		else if (isset($_GET['referer'])) header("location: ".$_GET['referer']);
	...
	case "pw_forgotten_ok":
		if (isset($pwf_username) && trim($pwf_username) != "" && isset($pwf_email) && trim($pwf_email) != "") {
			...
			if ($field["user_email"] == $pwf_email) {
				...
				header("location: ". basename($_SERVER['SCRIPT_NAME']) ."?msg=pwf_failed");
				die('<a href="'. basename($_SERVER['SCRIPT_NAME']) .'?msg=pwf_failed">further...</a>');
			}
		} else {
			header("location: ". basename($_SERVER['SCRIPT_NAME']) ."?msg=pwf_failed");
			die('<a href="'. basename($_SERVER['SCRIPT_NAME']) .'?msg=pwf_failed">further...</a>');
	...
	case "activate":
		if (isset($_GET['activate']) && trim($_GET['activate']) != "" && isset($_GET['code']) && trim($_GET['code']) != "") {
			...
			if ($field['user_id'] == $_GET["activate"] && $field['pwf_code'] == $_GET['code']) {
				...
				$new_user_pw = "";
				for ($i = 0; $i < 8; $i++) {
					$new_user_pw .= mb_substr($letters, mt_rand(0, mb_strlen($letters) - 1), 1);
				}
