...
$action = (isset($_GET['action']) and in_array($_GET['action'], array('uploaded', 'show_uploaded_images'))) ? $_GET['action'] : 'upload';
...
if (isset($_FILES['probe']) && $_FILES['probe']['size'] != 0 && !$_FILES['probe']['error']) {
	...
	if (!is_array($image_info) || !in_array($image_info[2], array(1, 2, 3))) $errors[] = $lang['invalid_file_format'];
		...
		if ($_FILES['probe']['size'] > $settings['upload_max_img_size']*1000 || $image_info[0] > $settings['upload_max_img_width'] || $image_info[1] > $settings['upload_max_img_height']) {
			...
			for ($compression = 100; $compression > 9; $compression = $compression - 10) {
				if (!resize($_FILES['probe']['tmp_name'], $uploaded_images_path.$img_tmp_name, $new_width, $new_height, $compression)) {
					$file_size = @filesize($uploaded_images_path.$img_tmp_name); break;
				}
...
<head>
<meta charset="utf-8">
<title><?php echo htmlsc($lang['upload_image_title']); ?></title>
<style type="text/css">
body {
